...
class WorldEditor:
    ...
    def set_sign(self, x, y, z, lines, facing="north", kind="oak", wall=False,
        ...
        """Place a real sign with engraved text. 1.21.5+ stores each line as an NBT text
        component (a plain string = literal text). `facing` = the readable side's direction."""
        ...
        def _face(lines4):
            return N.CompoundTag({
                "messages": N.ListTag([N.StringTag(str(s)) for s in lines4]),
                "color": N.StringTag(color),
                "has_glowing_text": N.ByteTag(1 if glowing else 0),
            })
        nbt = N.CompoundTag({
            "front_text": _face(msgs),
            "back_text": _face(["", "", "", ""]),
